...
produce_multi_topic_test() {
    ...
    for topic in $topics; do
        ...
        $kafka_bin_dir/kafka-producer-perf-test.sh --topic $topic --throughput -1 --num-records 122916666 --record-size 586 --producer-props bootstrap.servers=$kafka_bootstrap_servers | awk -v topic="$topic" -v time="$(date +'%Y-%m-%d %H:%M:%S')" '{print "" time " [" topic "] " $0}'
